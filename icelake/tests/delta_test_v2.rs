...
use icelake::io_v2::input_wrapper::DeltaWriter;
...
impl DeltaTest {
    ...
    async fn create_delta_writer(&self, table: &Table, equality_ids: Vec<usize>) -> DeltaWriter {
        ...
        let partition_eq_delta = builder_helper
            .fanout_partition_writer_builder(eq_delta)
            .unwrap();
    ...
    pub async fn test_write(&mut self) {
        ...
        let mut delta_writer = self.create_delta_writer(&table, vec![1, 2]).await;
        ...
        self.commit_writer(&mut table, &mut delta_writer).await;
